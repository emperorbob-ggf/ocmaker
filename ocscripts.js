let ctx, canvas;
let trion, attack, defense, mobility, skill, range, command, tactics, reactions, strategy, specialtech, recon, attacker, shooter, sniper, gunner;
...
const orderStat = ["TRION: ", "MOBILITY: ", "REACTIONS: ", "RECON: ", "STRATEGY: ", "SPECIAL TECHNIQUES: "];
...
function getParams() {
    ...
    reactions = Number(document.getElementById("reactions").value);
    strategy = Number(document.getElementById("strategy").value);
    specialtech = Number(document.getElementById("specialtech").value);
    recon = Number(document.getElementById("recon").value);
    attacker = Number(document.getElementById("attacker").value);
    shooter = Number(document.getElementById("shooter").value);
    sniper = Number(document.getElementById("sniper").value);
    gunner = Number(document.getElementById("gunner").value);
}
...
function draw() {
    ...
    const [tx, ty] = getTrion(trion);
    const [ax, ay] = getAttack(attack);
    const [dx, dy] = getDefense(defense);
    const [mx, my] = getMobility(mobility);
    const [sx, sy] = getSkill(skill);
    const [rx, ry] = getRange(range);
    const [cx, cy] = getCommand(command);
    const [ttx, tty] = getTactics(tactics);
